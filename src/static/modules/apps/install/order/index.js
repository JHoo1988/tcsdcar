...
define(['jquery', 'fastclick','rsvp','jea', 'weui','layer', 'ejs','calendar'], function ($, fastclick,Q,jea,weui,layer,ejs) {
    ...
    App.prototype = {
        ...
        init: function () {
            ...
            if (!data){
                var order = utilOrder.order.getOrder();
                if (order&&order.length>0){
                    data = order[0].productInfo;
                }else if(utilOrder.order.getOrderDetail()){
                    data = utilOrder.order.getOrderDetail().productInfo;
                }
            }
        ...
        renderPage: function (data,vipDiscountInfo) {
            var html = new EJS({url: '../../views/install/order/index.ejs'}).render({
                data : data,
        ...
        showCalendar: function () {
            ...
            if (products.length < 1) {
                products = utilOrder.order.getOrderDetail();
                if (!products){
                    //$.alert('请选择服务');
                    this.jumpTip();
                    return false;
