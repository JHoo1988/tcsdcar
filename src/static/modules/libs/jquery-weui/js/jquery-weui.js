...
+ function($) {
  ...
  $.modal = function(params, onOpen) {
    ...
    var buttonsHtml = buttons.map(function(d, i) {
      return '<a href="' + (d.href || "javascript:;") + '" class="weui_btn_dialog ' + (d.className || "") + '">' + d.text + '</a>';
    }).join("");
...
+ function($) {
  ...
  $.toastNoIcon = function(text, style) {
    var className;
    if(style == "noicon") {
      className = "weui_toast_null";
    }
    show('<p class="weui_toast_content">' + (text || "已经完成") + '</p>', className);
